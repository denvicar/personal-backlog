...
export default function GameList({games}) {
    ...
    return (<>
        <div className={"flex flex-row mx-auto lg:w-1/2 w-[70%] items-start gap-3 h-svh"}>
            <div className={"flex flex-col justify-between gap-2 m-auto max-h-svh "}>
                ...
                <div className={"overflow-y-scroll max-h-svh flex flex-col gap-2 w-full pb-2"}>
                    {displayedGames.map(g => <div onClick={() => handleRowClick(g.id)} key={g.id} className={"flex flex-row w-full items-center justify-around  "}>
